/*
 * uart_protocol.c
 *
 *  Created on: 1 lut 2026
 *      Author: 00ary
 */


#include "uart_protocol.h"
#include <string.h>
#include <stdio.h>
#include <stdlib.h>


static const uint8_t crc8_table[256] = {
    
};

/* ============ FUNKCJE POMOCNICZE ============ */

static uint8_t hex_char_to_val(char c) {
    if (c >= '0' && c <= '9') return c - '0';
    if (c >= 'A' && c <= 'F') return c - 'A' + 10;
    if (c >= 'a' && c <= 'f') return c - 'a' + 10;
    return 0;
}

static void val_to_hex(uint8_t val, char *out) {
    const char hex[] = "0123456789ABCDEF";
    out[0] = hex[(val >> 4) & 0x0F];
    out[1] = hex[val & 0x0F];
}

/* ============ FUNKCJE PUBLICZNE ============ */

void UartProtocol_Init(UartProtocol_t *proto, UART_HandleTypeDef *huart) {
    proto->huart = huart;
    proto->rx_index = 0;
    proto->rx_complete = false;
    proto->last_command = CMD_NONE;
    proto->command_arg = 0.0f;
    proto->running = true;

    memset(proto->rx_buffer, 0, UART_RX_BUFFER_SIZE);
    memset(proto->tx_buffer, 0, UART_TX_BUFFER_SIZE);
}

uint8_t UartProtocol_CRC8(const uint8_t *data, uint16_t len) {
    uint8_t crc = 0x00;

    for (uint16_t i = 0; i < len; i++) {
        crc = crc8_table[crc ^ data[i]];
    }

    return crc;
}

void UartProtocol_SendData(UartProtocol_t *proto,
                           float temp, float setpoint,
                           uint16_t heater_pwm, uint16_t fan_pwm,
                           uint32_t timestamp) {
    char msg[100];
    char crc_hex[3] = {0};

    // Formatuj wiadomość bez CRC
    int len = snprintf(msg, sizeof(msg),
                       "$DATA,%.2f,%.1f,%u,%u,%lu",
                       temp, setpoint, heater_pwm, fan_pwm, timestamp);

    // Oblicz CRC (od $ do końca danych)
    uint8_t crc = UartProtocol_CRC8((uint8_t*)msg, len);
    val_to_hex(crc, crc_hex);

    // Dodaj CRC i zakończenie
    snprintf(proto->tx_buffer, UART_TX_BUFFER_SIZE,
             "%s*%s\r\n", msg, crc_hex);

    // Wyślij
    HAL_UART_Transmit(proto->huart, proto->tx_buffer,
                      strlen((char*)proto->tx_buffer), 100);
}

void UartProtocol_SendPong(UartProtocol_t *proto) {
    char msg[] = "$PONG";
    char crc_hex[3] = {0};

    uint8_t crc = UartProtocol_CRC8((uint8_t*)msg, strlen(msg));
    val_to_hex(crc, crc_hex);

    snprintf(proto->tx_buffer, UART_TX_BUFFER_SIZE, "%s*%s\r\n", msg, crc_hex);
    HAL_UART_Transmit(proto->huart, proto->tx_buffer,
                      strlen((char*)proto->tx_buffer), 100);
}

void UartProtocol_SendAck(UartProtocol_t *proto, const char *cmd) {
    char msg[64];
    char crc_hex[3] = {0};

    int len = snprintf(msg, sizeof(msg), "$ACK,%s", cmd);
    uint8_t crc = UartProtocol_CRC8((uint8_t*)msg, len);
    val_to_hex(crc, crc_hex);

    snprintf(proto->tx_buffer, UART_TX_BUFFER_SIZE, "%s*%s\r\n", msg, crc_hex);
    HAL_UART_Transmit(proto->huart, proto->tx_buffer,
                      strlen((char*)proto->tx_buffer), 100);
}

void UartProtocol_SendNak(UartProtocol_t *proto, const char *error) {
    char msg[64];
    char crc_hex[3] = {0};

    int len = snprintf(msg, sizeof(msg), "$NAK,%s", error);
    uint8_t crc = UartProtocol_CRC8((uint8_t*)msg, len);
    val_to_hex(crc, crc_hex);

    snprintf(proto->tx_buffer, UART_TX_BUFFER_SIZE, "%s*%s\r\n", msg, crc_hex);
    HAL_UART_Transmit(proto->huart, proto->tx_buffer,
                      strlen((char*)proto->tx_buffer), 100);
}

void UartProtocol_ReceiveByte(UartProtocol_t *proto, uint8_t byte) {
    if (byte == '$') {
        // Początek nowej wiadomości
        proto->rx_index = 0;
        proto->rx_buffer[proto->rx_index++] = byte;
    }
    else if (byte == '\n') {
        // Koniec wiadomości
        proto->rx_buffer[proto->rx_index] = '\0';
        proto->rx_complete = true;
    }
    else if (byte != '\r' && proto->rx_index < UART_RX_BUFFER_SIZE - 1) {
        proto->rx_buffer[proto->rx_index++] = byte;
    }
}

bool UartProtocol_VerifyCRC(UartProtocol_t *proto) {
    // Znajdź pozycję '*'
    char *star = strchr((char*)proto->rx_buffer, '*');
    if (star == NULL) return false;

    // Pobierz CRC z wiadomości
    uint8_t received_crc = (hex_char_to_val(star[1]) << 4) | hex_char_to_val(star[2]);

    // Oblicz CRC danych (od $ do *)
    uint16_t data_len = star - (char*)proto->rx_buffer;
    uint8_t calculated_crc = UartProtocol_CRC8(proto->rx_buffer, data_len);

    return (received_crc == calculated_crc);
}

UartCommand_t UartProtocol_Parse(UartProtocol_t *proto) {
    if (!proto->rx_complete) {
        return CMD_NONE;
    }

    proto->rx_complete = false;
    proto->last_command = CMD_NONE;

    // Weryfikuj CRC
    if (!UartProtocol_VerifyCRC(proto)) {
        UartProtocol_SendNak(proto, "CRC");
        return CMD_NONE;
    }

    // Parsuj komendę
    char *cmd = (char*)proto->rx_buffer;

    if (strncmp(cmd, "$PING", 5) == 0) {
        proto->last_command = CMD_PING;
        UartProtocol_SendPong(proto);
    }
    else if (strncmp(cmd, "$SETP,", 6) == 0) {
        proto->last_command = CMD_SET_TEMP;
        proto->command_arg = atof(cmd + 6);
        UartProtocol_SendAck(proto, "SETP");
    }
    else if (strncmp(cmd, "$STAT", 5) == 0) {
        proto->last_command = CMD_GET_STATUS;
    }
    else if (strncmp(cmd, "$START", 6) == 0) {
        proto->last_command = CMD_START;
        proto->running = true;
        UartProtocol_SendAck(proto, "START");
    }
    else if (strncmp(cmd, "$STOP", 5) == 0) {
        proto->last_command = CMD_STOP;
        proto->running = false;
        UartProtocol_SendAck(proto, "STOP");
    }
    else if (strncmp(cmd, "$SEKP,", 6) == 0) {
        proto->last_command = CMD_SET_KP;
        proto->command_arg = atof(cmd + 6);
        UartProtocol_SendAck(proto, "SEKP");
    }
    else if (strncmp(cmd, "$SEKI,", 6) == 0) {
        proto->last_command = CMD_SET_KI;
        proto->command_arg = atof(cmd + 6);
        UartProtocol_SendAck(proto, "SEKI");
    }
    else if (strncmp(cmd, "$SEKD,", 6) == 0) {
        proto->last_command = CMD_SET_KD;
        proto->command_arg = atof(cmd + 6);
        UartProtocol_SendAck(proto, "SEKD");
    }
    else {
        UartProtocol_SendNak(proto, "UNKNOWN");
    }

    return proto->last_command;
}

float UartProtocol_GetArg(UartProtocol_t *proto) {
    return proto->command_arg;
}
